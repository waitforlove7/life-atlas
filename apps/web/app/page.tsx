...
function createPopup(place: Place): HTMLElement { const root = document.createElement('div'); root.className = 'place-popup'; const link = document.createElement('a'); link.href = '/places/' + place.id; const title = document.createElement('strong'); title.textContent = place.name; link.append(title); const location = document.createElement('span'); location.textContent = place.province + ', ' + place.country; const status = document.createElement('span'); status.className = 'place-popup-status'; status.style.backgroundColor = STATUS_COLORS[place.status]; status.textContent = place.status; root.append(link, location, status); return root; }
...
export default function Home() {
  ...
  const [level, setLevel] = useState<ViewLevel>('country'); const [countryName, setCountryName] = useState(''); const [provinceName, setProvinceName] = useState(''); const [selectedProvince, setSelectedProvince] = useState<SelectedProvince | null>(null); const [addingPlace, setAddingPlace] = useState(false); const [loading, setLoading] = useState(true); const [error, setError] = useState<string | null>(null);
  const [showStats, setShowStats] = useState(false); const [summary, setSummary] = useState<Summary | null>(null); const [breakdown, setBreakdown] = useState<StatusBreakdown[]>([]); const [timeline, setTimeline] = useState<TimelinePoint[]>([]); const [tags, setTags] = useState<TagStat[]>([]); const [statsLoading, setStatsLoading] = useState(false);

  const toggleStats = useCallback(function() { setShowStats(function(prev) { var next = !prev; localStorage.setItem(STATS_STORAGE_KEY, String(next)); return next; }); }, []);
  ...
  const loadStats = useCallback(async function() {
    ...
    try {
      var results = await Promise.all([
        ...
        getJson<TagStat[]>(API_URL + '/stats/tags'),
      ]);
      setSummary(results[0]); setBreakdown(results[1]); setTimeline(results[2]); setTags(results[3]);
  ...
  useEffect(function() {
    if (!mapContainer.current || map.current) return; var disposed = false;
    async function initialize() {
      var places: Place[] = []; var countryStats: CountryStat[] = []; var provinceStats: ProvinceStat[] = [];
      try { var all = await Promise.all([getJson<Place[]>(API_URL + '/places'), getJson<CountryStat[]>(API_URL + '/stats/countries'), getJson<ProvinceStat[]>(API_URL + '/stats/provinces')]); places = all[0]; countryStats = all[1]; provinceStats = all[2]; } catch { setError('Personal data is unavailable. The map is shown in offline mode.'); }
      if (disposed || !mapContainer.current) return;
      var visitedCountries = countryStats.filter(function(item) { return item.visited; }).map(function(item) { return item.iso_code; }); var visitedProvinces = new Set(provinceStats.filter(function(item) { return item.visited; }).map(function(item) { return item.country_iso + ':' + item.code; })); var selectedCountryIso = ''; var selectedCountryName = '';
      var instance = new maplibregl.Map({ container: mapContainer.current, style: { version: 8, sources: {}, layers: [{ id: 'background', type: 'background', paint: { 'background-color': '#f8fafc' } }] }, center: [20, 20], zoom: 1.5, attributionControl: false }); map.current = instance; instance.addControl(new maplibregl.NavigationControl({ showCompass: false }), 'top-right');
      var clearMarkers = function() { markers.current.forEach(function(marker) { marker.remove(); }); markers.current = []; }; var visibility = function(layers, visible) { layers.forEach(function(layer) { if (instance.getLayer(layer)) instance.setLayoutProperty(layer, 'visibility', visible ? 'visible' : 'none'); }); };
      var showWorld = function() { clearMarkers(); visibility(['provinces-fill', 'provinces-outline'], false); visibility(['countries-fill', 'countries-outline'], true); instance.flyTo({ center: [20, 20], zoom: 1.5 }); setLevel('country'); setCountryName(''); setProvinceName(''); setSelectedProvince(null); };
      var showProvince = function() { clearMarkers(); visibility(['countries-fill', 'countries-outline'], false); visibility(['provinces-fill', 'provinces-outline'], true); setLevel('province'); setProvinceName(''); setSelectedProvince(null); }; showWorldRef.current = showWorld; showProvinceRef.current = showProvince;
      var boundsFor = function(features) { var bounds = new maplibregl.LngLatBounds(); var visit = function(coordinates) { if (Array.isArray(coordinates) && typeof coordinates[0] === 'number') bounds.extend(coordinates); else if (Array.isArray(coordinates)) coordinates.forEach(visit); }; features.forEach(function(feature) { if (feature.geometry && 'coordinates' in feature.geometry) visit(feature.geometry.coordinates); }); return bounds; };
      var showPlaces = function(provinceCode, name, bounds) { clearMarkers(); setLevel('place'); setProvinceName(name); setSelectedProvince({ country: selectedCountryName, countryIso: selectedCountryIso, province: name, provinceCode: provinceCode }); places.filter(function(place) { return place.country_iso === selectedCountryIso && place.province_code === provinceCode; }).forEach(function(place) { var element = document.createElement('button'); element.className = 'place-marker'; element.style.backgroundColor = STATUS_COLORS[place.status]; element.setAttribute('aria-label', place.name); markers.current.push(new maplibregl.Marker({ element: element }).setLngLat([place.longitude, place.latitude]).setPopup(new maplibregl.Popup({ offset: 16 }).setDOMContent(createPopup(place))).addTo(instance)); }); if (!bounds.isEmpty()) instance.fitBounds(bounds, { padding: 64, maxZoom: 11 }); };
      var showCountry = async function(isoCode, name) { var data = await getJson<GeoJSON.FeatureCollection>('/data/admin1/' + isoCode + '.geojson'); data.features.forEach(function(feature) { if (feature.properties) feature.properties.visited = visitedProvinces.has(isoCode + ':' + String(feature.properties.code || '')); }); var source = instance.getSource('provinces'); if (source) source.setData(data); else { instance.addSource('provinces', { type: 'geojson', data: data }); instance.addLayer({ id: 'provinces-fill', type: 'fill', source: 'provinces', paint: { 'fill-color': ['case', ['boolean', ['get', 'visited'], false], LIT_COLOR, UNLIT_COLOR], 'fill-opacity': 0.72 } }); instance.addLayer({ id: 'provinces-outline', type: 'line', source: 'provinces', paint: { 'line-color': '#ffffff', 'line-width': 1 } }); } selectedCountryIso = isoCode; selectedCountryName = name; setCountryName(name); showProvince(); var bounds = boundsFor(data.features); if (!bounds.isEmpty()) instance.fitBounds(bounds, { padding: 48, maxZoom: 6 }); };
      instance.once('load', function() { instance.addSource('countries', { type: 'geojson', data: '/data/countries.geojson' }); instance.addLayer({ id: 'countries-fill', type: 'fill', source: 'countries', paint: { 'fill-color': ['case', ['in', ['get', 'iso_code'], ['literal', visitedCountries]], LIT_COLOR, UNLIT_COLOR], 'fill-opacity': 0.76 } }); instance.addLayer({ id: 'countries-outline', type: 'line', source: 'countries', paint: { 'line-color': '#ffffff', 'line-width': 0.8 } }); instance.on('click', 'countries-fill', async function(event) { var props = event.features?.[0]?.properties; if (!props?.iso_code) return; try { await showCountry(String(props.iso_code), String(props.name || props.iso_code)); } catch { setError('No first-level boundary data is available for ' + (props.name || props.iso_code) + '.'); } }); instance.on('click', 'provinces-fill', function(event) { var feature = event.features?.[0]; if (!feature?.properties?.code) return; showPlaces(String(feature.properties.code), String(feature.properties.name), boundsFor([feature])); }); ['countries-fill', 'provinces-fill'].forEach(function(layer) { instance.on('mouseenter', layer, function() { instance.getCanvas().style.cursor = 'pointer'; }); instance.on('mouseleave', layer, function() { instance.getCanvas().style.cursor = ''; }); }); setLoading(false); });
    }
  ...
  var maxTimelineCount = Math.max.apply(null, timeline.map(function(p) { return p.count; }).concat([1]));
  var maxCumulative = Math.max.apply(null, timeline.map(function(p) { return p.cumulative; }).concat([1]));
  var maxTagCount = Math.max.apply(null, tags.map(function(t) { return t.count; }).concat([1]));
    ...
    React.createElement('div', { className: 'atlas-panel' },
      React.createElement('header', { className: 'atlas-header-row' },
        React.createElement('div', null,
          ...
          React.createElement('p', null, 'Your Personal World Atlas')
        ),
      ...
      summary ? React.createElement(React.Fragment, null,
            ...
            React.createElement('div', { className: 'timeline-bars' },
              timeline.map(function(point) { return React.createElement('div', { key: point.month, className: 'timeline-column' },
                React.createElement('div', { className: 'timeline-bar-wrapper' },
                  React.createElement('div', { className: 'timeline-bar', style: { height: (point.count / maxTimelineCount * 100) + '%' }, title: formatMonth(point.month) + ': ' + point.count + ' place(s)' })
                ),
        ...
        tags.length > 0 && React.createElement('section', { className: 'stats-section' },
          React.createElement('h2', null, 'Tags'),
          React.createElement('div', { className: 'tag-cloud' },
            tags.map(function(tag) { return React.createElement('span', { key: tag.name, className: 'tag-cloud-item', style: { fontSize: (12 + (tag.count / maxTagCount) * 16) + 'px', opacity: 0.5 + (tag.count / maxTagCount) * 0.5 } },
              tag.name,
              React.createElement('small', null, tag.count)
            ); })
          )
        )
      ) : null
